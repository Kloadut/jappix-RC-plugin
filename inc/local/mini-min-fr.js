...
b64arrays();b64decode=function(s){return utf8d2t(b64t2d(s));}
b64encode=function(s){return b64d2t(utf8t2d(s));}
...
JSJaCMessage.prototype=new JSJaCPacket;JSJaCMessage.prototype.setBody=function(body){this._setChildNode("body",body);return this;};JSJaCMessage.prototype.setSubject=function(subject){this._setChildNode("subject",subject);return this;};JSJaCMessage.prototype.setThread=function(thread){this._setChildNode("thread",thread);return this;};JSJaCMessage.prototype.setNick=function(nick){var aNode=this.getChild("nick");var tNode=this.getDoc().createTextNode(nick);if(aNode)
try{aNode.replaceChild(tNode,aNode.firstChild);}catch(e){}
else{try{aNode=this.getDoc().createElementNS('http://jabber.org/protocol/nick',"nick");}catch(ex){aNode=this.getDoc().createElement("nick")}
this.getNode().appendChild(aNode);aNode.appendChild(tNode);}
...
b64encode(rPlain)+"</response>",this._doSASLAuthDigestMd5S2);}};JSJaCConnection.prototype._doSASLAuthDigestMd5S2=function(el){if(el.nodeName=='failure'){if(el.xml)
...
jQuery.removeData(element,this.dataKey);}}}});jQuery(window).bind("unload",function(){jQuery.each(jQuery.timer.global,function(index,item){jQuery.timer.remove(item);});});;(function($){var $scrollTo=$.scrollTo=function(target,duration,settings){$(window).scrollTo(target,duration,settings);};$scrollTo.defaults={axis:'xy',duration:parseFloat($.fn.jquery)>=1.3?0:1};$scrollTo.window=function(scope){return $(window)._scrollable();};$.fn._scrollable=function(){return this.map(function(){var elem=this,isWin=!elem.nodeName||$.inArray(elem.nodeName.toLowerCase(),['iframe','#document','html','body'])!=-1;if(!isWin)
...
function hasPersistent(){try{if(!window.localStorage)
return false;localStorage.setItem('haspersistent_check','ok');return true;}
catch(e){return false;}}
...
function strAfterLast(given_char,str){if(!given_char||!str)
return'';var char_index=str.lastIndexOf(given_char);var str_return=str;if(char_index>=0)
str_return=str.substr(char_index+1);return str_return;}
...
function bareXID(xid){xid=cutResource(xid);xid=xid.toLowerCase();return xid;}
...
return'0'+i;return i;}
function escapeRegex(query){return query.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,'\\$&');}
function randomArrayValue(arr){return arr[Math.floor(Math.random()*arr.length)];}
...
function relativeDate(to_parse){var current_date=Date.jab2date(getXMPPTime('utc'));var current_day=current_date.getDate();var current_stamp=current_date.getTime();var old_date=Date.jab2date(to_parse);var old_day=old_date.getDate();var old_stamp=old_date.getTime();var old_time=old_date.toLocaleTimeString();var days=Math.round((current_stamp-old_stamp)/86400000);if(isNaN(old_stamp)||isNaN(days))
return getCompleteTime();if(current_day==old_day)
...
var MINI_DISCONNECT=false;var MINI_AUTOCONNECT=false;var MINI_SHOWPANE=false;var MINI_INITIALIZED=false;var MINI_ANONYMOUS=false;var MINI_ANIMATE=false;var MINI_RANDNICK=false;var MINI_NICKNAME=null;var MINI_TITLE=null;var MINI_DOMAIN=null;var MINI_USER=null;var MINI_PASSWORD=null;var MINI_RECONNECT=0;var MINI_CHATS=[];var MINI_GROUPCHATS=[];var MINI_PASSWORDS=[];var MINI_RESOURCE=JAPPIX_RESOURCE+' Mini';var MINI_ERROR_LINK='https://mini.jappix.com/issues';function setupConMini(con){con.registerHandler('message',handleMessageMini);con.registerHandler('presence',handlePresenceMini);con.registerHandler('iq',handleIQMini);con.registerHandler('onerror',handleErrorMini);con.registerHandler('onconnect',connectedMini);}
...
return false;logThis('Jappix Mini is disconnecting...',3);MINI_DISCONNECT=true;MINI_INITIALIZED=false;con.registerHandler('ondisconnect',disconnectedMini);con.disconnect();return false;}
...
else{nick=jQuery('#jappix_mini a#friend-'+hash).text().revertHtmlEnc();if(!nick){var unknown_entry=jQuery("a.jm_unknown[data-xid="+xid+"]",jQuery("#jappix_mini"));if(unknown_entry.length>0){nick=unknown_entry.attr('data-nick');}else{msgnick=msg.getNick();nick=getXIDNick(xid);if(msgnick){if(nick!=msgnick){nick=msgnick+' ('+nick+')';}}
var unknown_entry=jQuery('<a class="jm_unknown jm_offline" href="#"></a>').attr('data-nick',nick).attr('data-xid',xid);unknown_entry.appendTo(jQuery(".jm_buddies",jQuery("#jappix_mini")));}}}
...
var groupchat_path='#jappix_mini #chat-'+hash+'[data-type=groupchat]';var is_groupchat=false;if(exists(groupchat_path)){is_groupchat=true;if(resource!=unescape(jQuery(groupchat_path).attr('data-nick'))){var groupchat=xid;xid=from;hash=hex_md5(xid);if(show=='unavailable')
removeBuddyMini(hash,groupchat);else
addBuddyMini(xid,hash,resource,groupchat);}}
var chat='#jappix_mini #chat-'+hash;var friend='#jappix_mini a#friend-'+hash;var send_input=chat+' input.jm_send-messages';if(show=='unavailable'){jQuery(friend).addClass('jm_offline').removeClass('jm_online jm_hover');jQuery(friend).hide();if(is_groupchat){jQuery(chat).addClass('jm_disabled');jQuery(send_input).blur().attr('disabled',true).attr('data-value',_e("Déconnecté(e)")).val(_e("Déconnecté(e)"));}}
else{jQuery(friend).removeClass('jm_offline').addClass('jm_online');var search=jQuery('#jappix_mini div.jm_roster div.jm_search input.jm_searchbox').val();var regex=new RegExp('((^)|( ))'+escapeRegex(search),'gi');var nick=unescape(jQuery(friend).data('nick'));if(search&&!nick.match(regex))
jQuery(friend).hide();if(is_groupchat){jQuery(chat).removeClass('jm_disabled');jQuery(send_input).removeAttr('disabled').val('');}}
...
function sendMessageMini(aForm){try{var body=trim(aForm.body.value);var xid=aForm.xid.value;var type=aForm.type.value;var hash=hex_md5(xid);if(body&&xid){var aMsg=new JSJaCMessage();var known_roster_entry=jQuery("a.jm_friend[data-xid="+xid+"]",jQuery("#jappix_mini"));if(0==known_roster_entry.length){var subscription=known_roster_entry.attr('data-sub');if('both'!=subscription&&'from'!=subscription){aMsg.setNick(MINI_NICKNAME);}}
aMsg.setTo(xid);aMsg.setType(type);aMsg.setBody(body);con.send(aMsg);aForm.body.value='';if(type!='groupchat')
displayMessageMini(type,body,getXID(),'me',hash,getCompleteTime(),getTimeStamp(),'user-message');logThis('Message ('+type+') sent to: '+xid);}}
catch(e){}
finally{return false;}}
function smileyMini(image,text){return' <img class="jm_smiley jm_smiley-'+image+' jm_images" alt="'+encodeQuotes(text)+'" src="plugins/jappix/inc/local/blank.gif" /> ';}
...
jQuery(tab).append('<span class="jm_notify">'+'<span class="jm_notify_left jm_images"></span>'+'<span class="jm_notify_middle">0</span>'+'<span class="jm_notify_right jm_images"></span>'+'</span>');var number=parseInt(jQuery(notify_middle).text());jQuery(notify_middle).text(number+1);notifyCountersMini();}
function notifyErrorMini(){jQuery('#jappix_mini').html('<div class="jm_starter">'+'<a class="jm_pane jm_button jm_images" href="'+MINI_ERROR_LINK+'" target="_blank" title="'+_e("Cliquez ici pour résoudre l\'erreur")+'">'+'<span class="jm_counter jm_error jm_images">'+_e("Erreur")+'</span>'+'</a>'+'</div>');}
function notifyCountersMini(){var number=0;jQuery('#jappix_mini span.jm_notify span.jm_notify_middle').each(function(){number=number+parseInt(jQuery(this).text());});jQuery('#jappix_mini a.jm_switch').removeClass('jm_notifnav');if(number){if(jQuery('#jappix_mini div.jm_conversation:visible:first').prevAll().find('span.jm_notify').size())
jQuery('#jappix_mini a.jm_switch.jm_left').addClass('jm_notifnav');if(jQuery('#jappix_mini div.jm_conversation:visible:last').nextAll().find('span.jm_notify').size())
jQuery('#jappix_mini a.jm_switch.jm_right').addClass('jm_notifnav');}
if(MINI_TITLE==null)
return;var title=MINI_TITLE;if(number)
title='['+number+'] '+title;document.title=title;return;}
function clearNotificationsMini(hash){if(!isFocused())
return false;jQuery('#jappix_mini #chat-'+hash+' span.jm_notify').remove();notifyCountersMini();return true;}
function updateRosterMini(){jQuery('#jappix_mini a.jm_button span.jm_counter').text(jQuery('#jappix_mini a.jm_online').size());}
function updateOverflowMini(){jQuery('#jappix_mini div.jm_conversation:hidden').show();var number_visible=parseInt((jQuery(window).width()-330)/140);var number_total=jQuery('#jappix_mini div.jm_conversation').size();if(number_visible<=0)
number_visible=1;if(number_visible<number_total){if(!jQuery('#jappix_mini a.jm_switch').size()){jQuery('#jappix_mini div.jm_conversations').before('<a class="jm_switch jm_left jm_pane jm_images" href="#">'+'<span class="jm_navigation jm_images"></span>'+'</a>');jQuery('#jappix_mini div.jm_conversations').after('<a class="jm_switch jm_right jm_pane jm_images" href="#">'+'<span class="jm_navigation jm_images"></span>'+'</a>');overflowEventsMini();}
var index_visible=number_visible-1;jQuery('#jappix_mini div.jm_conversation:gt('+index_visible+'):visible').hide();if(jQuery('#jappix_mini div.jm_conversation:hidden a.jm_pane.jm_clicked').size())
switchPaneMini();jQuery('#jappix_mini a.jm_switch').removeClass('jm_nonav');if(!jQuery('#jappix_mini div.jm_conversation:visible:first').prev().size())
jQuery('#jappix_mini a.jm_switch.jm_left').addClass('jm_nonav');if(!jQuery('#jappix_mini div.jm_conversation:visible:last').next().size())
jQuery('#jappix_mini a.jm_switch.jm_right').addClass('jm_nonav');}
else{jQuery('#jappix_mini a.jm_switch').remove();jQuery('#jappix_mini div.jm_conversation:hidden').show();}}
function overflowEventsMini(){jQuery('#jappix_mini a.jm_switch').click(function(){if(jQuery(this).hasClass('jm_nonav'))
return false;var hide_this=show_this='';if(jQuery(this).is('.jm_left')){show_this=jQuery('#jappix_mini div.jm_conversation:visible:first').prev();if(show_this.size())
hide_this=jQuery('#jappix_mini div.jm_conversation:visible:last');}
else{show_this=jQuery('#jappix_mini div.jm_conversation:visible:last').next();if(show_this.size())
hide_this=jQuery('#jappix_mini div.jm_conversation:visible:first');}
if(show_this&&show_this.size()){if(hide_this&&hide_this.size()){hide_this.hide();if(hide_this.find('a.jm_pane').hasClass('jm_clicked'))
switchPaneMini();}
show_this.show();jQuery('#jappix_mini a.jm_switch').removeClass('jm_nonav');if((jQuery(this).is('.jm_right')&&!show_this.next().size())||(jQuery(this).is('.jm_left')&&!show_this.prev().size()))
jQuery(this).addClass('jm_nonav');notifyCountersMini();}
return false;});}
...
else{dom='<div class="jm_position">'+'<div class="jm_conversations"></div>'+'<div class="jm_starter">'+'<div class="jm_roster">'+'<div class="jm_actions">'+'<a class="jm_logo jm_images" href="https://mini.jappix.com/" target="_blank"></a>'+'<a class="jm_one-action jm_join jm_images" title="'+_e("Rejoindre une discussion")+'" href="#"></a>'+'<a class="jm_one-action jm_status" title="'+_e("Statut")+'" href="#">'+'<span class="jm_presence jm_images jm_available"></span>'+'</a>'+'<div class="jm_status_picker">'+'<a href="#" data-status="available">'+'<span class="jm_presence jm_images jm_available"></span>'+'<span class="jm_show_text">'+_e("Disponible")+'</span>'+'</a>'+'<a href="#" data-status="away">'+'<span class="jm_presence jm_images jm_away"></span>'+'<span class="jm_show_text">'+_e("Absent(e)")+'</span>'+'</a>'+'<a href="#" data-status="dnd">'+'<span class="jm_presence jm_images jm_dnd"></span>'+'<span class="jm_show_text">'+_e("Occupé(e)")+'</span>'+'</a>'+'<a href="#" data-status="unavailable">'+'<span class="jm_presence jm_images jm_unavailable"></span>'+'<span class="jm_show_text">'+_e("Déconnecté(e)")+'</span>'+'</a>'+'</div>'+'</div>'+'<div class="jm_buddies"></div>'+'<div class="jm_search">'+'<input type="text" class="jm_searchbox jm_images" placeholder="'+_e("Filtrer")+'" data-value="" />'+'</div>'+'</div>'+'<a class="jm_pane jm_button jm_images" href="#">'+'<span class="jm_counter jm_images">'+_e("Patientez...")+'</span>'+'</a>'+'</div>'+'</div>';}
jQuery('body').append('<div id="jappix_mini">'+dom+'</div>');jQuery('#jappix_mini a.jm_status').removeClass('active');jQuery('#jappix_mini div.jm_status_picker').hide();adaptRosterMini();overflowEventsMini();updateOverflowMini();jQuery('#jappix_mini div.jm_starter, #jappix_mini div.jm_conversations, #jappix_mini div.jm_conversation, #jappix_mini a.jm_switch').css('float','left');jQuery('#jappix_mini a.jm_button').click(function(){try{var counter='#jappix_mini a.jm_pane.jm_button span.jm_counter';if(jQuery(counter).text()==_e("Patientez..."))
...
finally{return false;}});jQuery('#jappix_mini a.jm_status').click(function(){try{if(jQuery(this).hasClass('active')){jQuery('#jappix_mini div.jm_status_picker').hide();jQuery(this).blur().removeClass('active');}else{jQuery('#jappix_mini div.jm_status_picker').show();jQuery(this).addClass('active');}}
catch(e){}
finally{return false;}});jQuery('#jappix_mini div.jm_status_picker a').click(function(){try{var pr_xid=[''];jQuery('#jappix_mini div.jm_conversation[data-type=groupchat]').each(function(){pr_xid.push(jQuery(this).attr('data-xid'));});var new_status=jQuery(this).data('status');jQuery.each(pr_xid,function(key,value){switch(new_status){case'available':presenceMini('','','','',value);break;case'away':presenceMini('','away','','',value);break;case'dnd':presenceMini('','dnd','','',value);break;case'unavailable':disconnectMini();break;default:presenceMini('','','','',value);break;}});if(new_status!='unavailable'){jQuery('#jappix_mini a.jm_status span').removeClass('jm_available jm_away jm_dnd jm_unavailable').addClass('jm_'+jQuery(this).data('status'));jQuery('#jappix_mini div.jm_status_picker').hide();jQuery('#jappix_mini a.jm_status').blur().removeClass('active');}}
catch(e){}
...
finally{return false;}});jQuery('#jappix_mini div.jm_roster div.jm_search input.jm_searchbox').keyup(function(e){if((e.keyCode==38)||(e.keyCode==40))
return;jQuery(this).attr('data-value',jQuery(this).val());var self=this;typewatch(function(){try{var search=jQuery(self).val();var regex=new RegExp('((^)|( ))'+escapeRegex(search),'gi');jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery('#jappix_mini div.jm_roster div.jm_grouped').show();if(!search){jQuery('#jappix_mini div.jm_roster div.jm_buddies a.jm_online').show();return;}
jQuery('#jappix_mini div.jm_roster div.jm_buddies a.jm_online').each(function(){var nick=unescape(jQuery(this).data('nick'));if(nick.match(regex))
jQuery(this).show();else
jQuery(this).hide();});jQuery('#jappix_mini div.jm_roster div.jm_grouped').each(function(){if(!jQuery(this).find('a.jm_online:visible').size())
jQuery(this).hide();});jQuery('#jappix_mini div.jm_roster div.jm_buddies a.jm_online:visible:first').addClass('jm_hover');}
catch(e){}
finally{return false;}},500);});jQuery(document).keydown(function(e){if(jQuery('#jappix_mini div.jm_roster').is(':hidden'))
return;if((e.keyCode==38)||(e.keyCode==40)){if(!jQuery('#jappix_mini a.jm_online.jm_hover').size()){if(e.keyCode==38)
jQuery('#jappix_mini a.jm_online:visible:last').addClass('jm_hover');else
jQuery('#jappix_mini a.jm_online:visible:first').addClass('jm_hover');}
else if(jQuery('#jappix_mini a.jm_online:visible').size()>1){var hover_index=jQuery('#jappix_mini a.jm_online:visible').index(jQuery('a.jm_hover'));if(e.keyCode==38)
hover_index--;else
hover_index++;if(!hover_index)
hover_index=0;if(!jQuery('#jappix_mini a.jm_online:visible').eq(hover_index).size()){if(e.keyCode==38)
hover_index=jQuery('#jappix_mini a.jm_online:visible:last').index();else
hover_index=0;}
jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery('#jappix_mini a.jm_online:visible').eq(hover_index).addClass('jm_hover');}
jQuery('#jappix_mini div.jm_roster div.jm_buddies').scrollTo(jQuery('#jappix_mini a.jm_online.jm_hover'),0,{margin:true});return false;}
if((e.keyCode==13)&&jQuery('#jappix_mini a.jm_friend.jm_hover').size()){jQuery('#jappix_mini a.jm_friend.jm_hover').click();return false;}});jQuery(document).click(function(evt){if(!jQuery(evt.target).parents('#jappix_mini').size()&&!exists('#jappix_popup'))
...
jQuery(this).val(chat_value);});var search_box=jQuery('#jappix_mini div.jm_roster div.jm_search input.jm_searchbox');var filter_value=search_box.attr('data-value');if(filter_value)
search_box.val(filter_value).keyup();jQuery('#jappix_mini a.jm_friend').click(function(){try{chatMini('chat',unescape(jQuery(this).attr('data-xid')),unescape(jQuery(this).attr('data-nick')),jQuery(this).attr('data-hash'));}
catch(e){}
finally{return false;}});jQuery('#jappix_mini a.jm_friend').hover(function(){jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery(this).addClass('jm_hover');},function(){jQuery(this).removeClass('jm_hover');});jQuery('#jappix_mini a.jm_friend').mousedown(function(){jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery(this).addClass('jm_hover');});jQuery('#jappix_mini a.jm_friend').focus(function(){jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery(this).addClass('jm_hover');});jQuery('#jappix_mini a.jm_friend').blur(function(){jQuery(this).removeClass('jm_hover');});jQuery('#jappix_mini div.jm_conversation').each(function(){chatEventsMini(jQuery(this).attr('data-type'),unescape(jQuery(this).attr('data-xid')),jQuery(this).attr('data-hash'));});var scroll_hash=jQuery('#jappix_mini div.jm_conversation:has(a.jm_pane.jm_clicked)').attr('data-hash');var scroll_position=getDB('jappix-mini','scroll');if(scroll_position)
...
header+='<b class="jm_name" style="color: '+generateColor(nick)+';" data-xid="'+encodeQuotes(xid)+'">'+nick.htmlEnc()+'</b>';else if(nick=='me')
header+='<b class="jm_name jm_me" data-xid="'+encodeQuotes(xid)+'">'+_e("Vous")+'</b>';else
...
body=body.htmlEnc();body=body.replace(/(;-?\))(\s|$)/gi,smileyMini('wink','$1')).replace(/(:-?3)(\s|$)/gi,smileyMini('waii','$1')).replace(/(:-?\()(\s|$)/gi,smileyMini('unhappy','$1')).replace(/(:-?P)(\s|$)/gi,smileyMini('tongue','$1')).replace(/(:-?O)(\s|$)/gi,smileyMini('surprised','$1')).replace(/(:-?\))(\s|$)/gi,smileyMini('smile','$1')).replace(/(\^_?\^)(\s|$)/gi,smileyMini('happy','$1')).replace(/(:-?D)(\s|$)/gi,smileyMini('grin','$1'));body=body.replace(/(^|\s|>|\()((\*)([^<>'"\*]+)(\*))($|\s|<|\))/gi,'$1<b>$2</b>$6').replace(/(^|\s|>|\()((\/)([^<>'"\/]+)(\/))($|\s|<|\))/gi,'$1<em>$2</em>$6').replace(/(^|\s|>|\()((_)([^<>'"_]+)(_))($|\s|<|\))/gi,'$1<span style="text-decoration: underline;">$2</span>$6');body=applyLinks(body,'mini');if(me_command)
...
function switchPaneMini(element,hash){hideRosterMini();jQuery('#jappix_mini a.jm_pane').removeClass('jm_clicked');jQuery('#jappix_mini div.jm_chat-content').hide();if(element&&(element!='roster')){var current='#jappix_mini #'+element;if(jQuery(current).size()&&jQuery(current).is(':hidden')){var click_nav='';if(jQuery('#jappix_mini div.jm_conversation:visible:first').prevAll().is('#'+element))
click_nav=jQuery('#jappix_mini a.jm_switch.jm_left');else
click_nav=jQuery('#jappix_mini a.jm_switch.jm_right');if(click_nav){while(jQuery(current).is(':hidden')&&!click_nav.hasClass('jm_nonav'))
click_nav.click();}}
jQuery(current+' a.jm_pane').addClass('jm_clicked');jQuery(current+' div.jm_chat-content').show();jQuery(document).oneTime(10,function(){jQuery(current+' input.jm_send-messages').focus();});if(hash)
messageScrollMini(hash);}}
...
function chatMini(type,xid,nick,hash,pwd,show_pane){var current='#jappix_mini #chat-'+hash;if(!exists(current)){if(type=='groupchat'){if(!MINI_NICKNAME&&MINI_RANDNICK)
MINI_NICKNAME=randomNickMini();var nickname=MINI_NICKNAME;if(!nickname){openPromptMini(printf(_e("Veuillez entrer votre pseudonyme pour rejoindre %s."),xid));jQuery('#jappix_popup div.jm_prompt form').submit(function(){try{var nickname=closePromptMini();if(nickname)
MINI_NICKNAME=nickname;chatMini(type,xid,nick,hash,pwd);}
catch(e){}
finally{return false;}});return;}}
var html='<div class="jm_conversation jm_type_'+type+'" id="chat-'+hash+'" data-xid="'+escape(xid)+'" data-type="'+type+'" data-nick="'+escape(nick)+'" data-hash="'+hash+'" data-origin="'+escape(cutResource(xid))+'">'+'<div class="jm_chat-content">'+'<div class="jm_actions">'+'<span class="jm_nick">'+nick+'</span>';var groupchat_exists=false;var chat_exists=false;if((type=='groupchat')&&MINI_GROUPCHATS&&MINI_GROUPCHATS.length){for(g in MINI_GROUPCHATS){if(xid==bareXID(generateXID(MINI_GROUPCHATS[g],'groupchat'))){groupchat_exists=true;break;}}}
if((type=='chat')&&MINI_CHATS&&MINI_CHATS.length){for(c in MINI_CHATS){if(xid==bareXID(generateXID(MINI_CHATS[c],'chat'))){chat_exists=true;break;}}}
if(((type=='groupchat')&&!groupchat_exists)||((type=='chat')&&!chat_exists)||((type!='groupchat')&&(type!='chat')))
html+='<a class="jm_one-action jm_close jm_images" title="'+_e("Fermer")+'" href="#"></a>';html+='</div>'+'<div class="jm_received-messages" id="received-'+hash+'"></div>'+'<form action="#" method="post">'+'<input type="text" class="jm_send-messages" name="body" autocomplete="off" placeholder="'+_e("Discussion")+'" data-value="" />'+'<input type="hidden" name="xid" value="'+xid+'" />'+'<input type="hidden" name="type" value="'+type+'" />'+'</form>'+'</div>'+'<a class="jm_pane jm_chat-tab jm_images" href="#">'+'<span class="jm_name">'+nick.htmlEnc()+'</span>'+'</a>'+'</div>';jQuery('#jappix_mini div.jm_conversations').prepend(html);if(type!='groupchat'){var selector=jQuery('#jappix_mini a#friend-'+hash+' span.jm_presence');var show='available';if(selector.hasClass('jm_unavailable')||!selector.size())
...
if(show_pane!=false)
jQuery(document).oneTime(10,function(){switchPaneMini('chat-'+hash,hash);});updateOverflowMini();return false;}
...
catch(e){}});jQuery(current+' input.jm_send-messages').focus(function(){clearNotificationsMini(hash);}).keyup(function(){jQuery(this).attr('data-value',jQuery(this).val());}).keydown(function(e){if(e.keyCode==9){switchChatMini();return false;}
if(e.keyCode==27){if(jQuery(current+' a.jm_close').size()){if(jQuery(current).next('div.jm_conversation').size())
jQuery(current).next('div.jm_conversation').find('a.jm_pane').click();else if(jQuery(current).prev('div.jm_conversation').size())
jQuery(current).prev('div.jm_conversation').find('a.jm_pane').click();jQuery(current+' a.jm_close').click();}
return false;}});}
function switchChatMini(){if(jQuery('#jappix_mini div.jm_conversation').size()<=1)
return;var chat_index=jQuery('#jappix_mini div.jm_conversation:has(a.jm_pane.jm_clicked)').index();chat_index++;if(!chat_index)
chat_index=0;if(!jQuery('#jappix_mini div.jm_conversation').eq(chat_index).size())
chat_index=0;while(jQuery('#jappix_mini div.jm_conversation').eq(chat_index).hasClass('jm_disabled'))
chat_index++;var chat_hash=jQuery('#jappix_mini div.jm_conversation').eq(chat_index).attr('data-hash');if(chat_hash)
switchPaneMini('chat-'+chat_hash,chat_hash);}
function showRosterMini(){switchPaneMini('roster');jQuery('#jappix_mini div.jm_roster').show();jQuery('#jappix_mini a.jm_button').addClass('jm_clicked');jQuery(document).oneTime(10,function(){jQuery('#jappix_mini div.jm_roster div.jm_search input.jm_searchbox').focus();});}
function hideRosterMini(){jQuery('#jappix_mini a.jm_status.active').click();jQuery('#jappix_mini div.jm_roster').hide();jQuery('#jappix_mini a.jm_button').removeClass('jm_clicked');jQuery('#jappix_mini div.jm_roster div.jm_search input.jm_searchbox').val('').attr('data-value','');jQuery('#jappix_mini div.jm_roster div.jm_grouped').show();jQuery('#jappix_mini div.jm_roster div.jm_buddies a.jm_online').show();jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');}
...
for(var j=0;j<MINI_CHATS.length;j++){if(!MINI_CHATS[j])
continue;try{var chat_xid=bareXID(generateXID(MINI_CHATS[j],'chat'));var chat_hash=hex_md5(chat_xid);var chat_nick=jQuery('#jappix_mini a#friend-'+chat_hash).attr('data-nick');if(!chat_nick)
chat_nick=getXIDNick(chat_xid);else
chat_nick=unescape(chat_nick);chatMini('chat',chat_xid,chat_nick,chat_hash);}
catch(e){}}
if(!MINI_AUTOCONNECT&&!MINI_GROUPCHATS.length&&!MINI_CHATS.length)
jQuery(document).oneTime(10,function(){showRosterMini();});}
function addBuddyMini(xid,hash,nick,groupchat,subscription){var element='#jappix_mini a.jm_friend#friend-'+hash;if(exists(element))
...
if(subscription){substr=' data-sub="'+subscription+'" ';}else{substr='';}
...
+nick.htmlEnc()+'</a>';if(groupchat)
jQuery(path).append(code);else
jQuery(path).prepend(code);jQuery(element).click(function(){try{chatMini('chat',xid,nick,hash);}
catch(e){}
finally{return false;}});jQuery(element).hover(function(){jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery(this).addClass('jm_hover');},function(){jQuery(this).removeClass('jm_hover');});jQuery('#jappix_mini a.jm_friend').mousedown(function(){jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery(this).addClass('jm_hover');});jQuery(element).focus(function(){jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');jQuery(this).addClass('jm_hover');});jQuery(element).blur(function(){jQuery(this).removeClass('jm_hover');});return true;}
...
function handleRosterMini(iq){var buddies=[];var i=0;jQuery(iq.getQuery()).find('item').each(function(){var current=jQuery(this);var xid=current.attr('jid');var subscription=current.attr('subscription');if(!isGateway(xid)){var nick=current.attr('name');var hash=hex_md5(xid);buddies[i]=[];if(!nick)
nick=getXIDNick(xid);buddies[i][0]=nick;buddies[i][1]=hash;buddies[i][2]=xid;buddies[i][3]=subscription;}
i++;});var buddies=buddies.sort();var x=buddies.length;var nick,hash,xid,subscription;for(var i=0;i<x;i++){nick=buddies[i][0];hash=buddies[i][1];xid=buddies[i][2];subscription=buddies[i][3];if(subscription=='remove')
removeBuddyMini(hash);else
addBuddyMini(xid,hash,nick,null,subscription);}
if(!MINI_INITIALIZED)
initializeMini();logThis('Roster got.',3);}
function adaptRosterMini(){var height=jQuery(window).height()-85;jQuery('#jappix_mini div.jm_roster div.jm_buddies').css('max-height',height);}
function randomNickMini(){var first_arr=['Just','Bob','Jar','Pedr','Yod','Maz','Vez','Car','Erw','Tiet','Iot','Wal','Bez','Pop','Klop','Zaz','Yoy','Raz'];var second_arr=['io','ice','a','u','o','ou','oi','ana','oro','izi','ozo','aza','ato','ito','ofa','oki','ima','omi'];var last_arr=['t','z','r','n','tt','zz','pp','j','k','v','c','x','ti','to','ta','ra','ro','ri'];var rand_nick=randomArrayValue(first_arr)+randomArrayValue(second_arr)+randomArrayValue(last_arr);return rand_nick;}
var typewatch=(function(){var timer=0;return function(callback,ms){clearTimeout(timer);timer=setTimeout(callback,ms);}})();function launchMini(autoconnect,show_pane,domain,user,password){MINI_DOMAIN=domain;MINI_USER=user;MINI_PASSWORD=password;if(!user||!password)
...
jQuery('head').append('<link rel="stylesheet" href="plugins/jappix/inc/local/mini-min.css" type="text/css" media="all" />');if((BrowserDetect.browser=='Explorer')&&(BrowserDetect.version<7))
jQuery('head').append('<link rel="stylesheet" href="plugins/jappix/inc/local/mini-min-ie.css" type="text/css" media="all" />');jQuery(document).keydown(function(e){if((e.keyCode==27)&&!isDeveloper())
return false;});MINI_TITLE=document.title;jQuery(window).resize(function(){adaptRosterMini();updateOverflowMini();});if(BrowserDetect.browser=='Opera'){jQuery('a[href]:not([onclick])').click(function(){var href=jQuery(this).attr('href')||'';var target=jQuery(this).attr('target')||'';if(href&&!href.match(/^#/i)&&!target.match(/_blank|_new/i))
